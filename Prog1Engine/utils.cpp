...
#pragma region textureImplementations
	...
	void DrawTexture(const Texture& texture, const Point2f& dstTopLeft, float rotation, const Rectf& srcRect)
	{
		Rectf dstRect{ dstTopLeft.x, dstTopLeft.y, srcRect.width, srcRect.height };
		DrawTexture(texture, dstRect, rotation, srcRect);
	}


	void DrawTexture(const Texture& texture, const Rectf& dstRect, float rotation, const Rectf& srcRect)
	{
...
#pragma region CollisionFunctionality
	...
	{
		return sqrtf(abs(x2 - x1) * abs(x2 - x1) + abs(y2 - y1) * abs(y2 - y1));
	}
	...
	{
		return (GetDistance(centerX, centerY, pointX, pointY) <= radius);
	}
	bool IsPointInCircle(const Point2f& centerPos, const float& radius, const Point2f& pointPos)
	{
		return IsPointInCircle(centerPos.x, centerPos.y, radius, pointPos.x, pointPos.y);
	}
	bool IsPointInCircle(const Circlef& circle, const Point2f& pointPos)
	{
		return IsPointInCircle(circle.center.x, circle.center.y, circle.radius, pointPos.x, pointPos.y);
	}
	...
	{
		return IsPointInRect(pos.x, pos.y, width, height, pointPos.x, pointPos.y);
	}
	bool IsPointInRect(const Rectf& rect, const Point2f& pointPos)
	{
		return IsPointInRect(rect.left, rect.top, rect.width, rect.height, pointPos.x, pointPos.y);
	}
	...
	{
		return(GetDistance(centerX1, centerY1, centerX2, centerY2) < radius1 + radius2);
	}
	bool IsOverlapping(const Point2f& centerPos1, const float& radius1,
		const Point2f& centerPos2, const float& radius2)
	{
		return IsOverlapping(centerPos1.x, centerPos1.y, radius1,
			centerPos2.x, centerPos2.y, radius2);
	}
	bool IsOverlapping(const Circlef& circle1, const Circlef& circle2)
	{
		return IsOverlapping(circle1.center.x, circle1.center.y, circle1.radius,
		...
		const float& left2, const float& top2, const float& width2, const float& height2)
	{
		...
		if (top1 > top2 + height2 || top2 > top1 + height1)
			return false;
	...
	{
		return IsOverlapping(rectPos1.x, rectPos1.y, width1, height1,
			rectPos2.x, rectPos2.y, width2, height2);
	}
	bool IsOverlapping(const Rectf& rect1, const Rectf& rect2)
	{
		return IsOverlapping(rect1.left, rect1.top, rect1.width, rect1.height,
			rect2.left, rect2.top, rect2.width, rect2.height);
	}
	bool IsOverlapping(const Rectf& rect, const Circlef& circle)
	{
		return IsOverlapping(circle.radius, circle.center.x, circle.center.y,rect.left, rect.top,
			rect.left + rect.width, rect.top + rect.height);
	}
	bool IsOverlapping(const float& radius, const float& circleX, const float& circleY, const float& RectX1, const float& RectY1, const float& RectX2, const float& RectY2)
	{
		const Point2f closestPoint{
			fmax(RectX1, fmin(circleX, RectX2)),
			fmax(RectY1, fmin(circleY, RectY2))
		};

		const float Dx{ closestPoint.x - circleX },
			Dy{ closestPoint.y - circleY };
